...
namespace fastllm {
    ...
    bool BaseAscendOperator::CompileAndRunSingleOp(const std::string &opType, const fastllm::OrderedData &inputData, const fastllm::DataDict &outputData,
                                                   const DynamicShapeDict &dynamicShapes, const fastllm::FloatDict &floatParams, 
                                                   const fastllm::IntDict &intParams, const fastllm::BoolDict &boolParams) {
        std::vector<aclTensorDesc *> inputTensors;
        std::vector<aclDataBuffer *> inputBuffers;
        for (auto &pair : inputData)
            npu::FastllmAclToTensor(pair, inputTensors, inputBuffers);
