...
namespace fastllm {
    GLM4Model::GLM4Model()
        ...
        weight.linearNames = {
            "lm_head.weight", "model.layers.*.mlp.down_proj.weight", "model.layers.*.mlp.gate_proj.weight", "model.layers.*.mlp.up_proj.weight",
            "model.layers.*.self_attn.o_proj.weight", "model.layers.*.self_attn.q_proj.weight", "model.layers.*.self_attn.k_proj.weight", "model.layers.*.self_attn.v_proj.weight"
        };
    }

    void GLM4Model::InitParams() {
    ...
    int GLM4Model::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
    ...
    std::vector <int> GLM4Model::ForwardBatch(int batch, const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
                                const fastllm::Data &positionIds, std::vector<std::pair<Data, Data>> &pastKeyValues,
                                const GenerationConfig &generationConfig, const LastTokensManager &lastTokens,
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            // post self-attention layernorm
            std::string postAttnNormName = "model.layers." + std::to_string(i) + ".post_self_attn_layernorm.weight";
            if (weight.weight.find(postAttnNormName) != weight.weight.end())
                RMSNorm(attenInput, this->weight[postAttnNormName], rms_norm_eps, attenInput);
            ...
            std::string postMLPNormName = "model.layers." + std::to_string(i) + ".post_mlp_layernorm.weight";
            if (weight.weight.find(postMLPNormName) != weight.weight.end())
                RMSNorm(k, this->weight[postMLPNormName], rms_norm_eps, k);
    ...
    std::vector <int> GLM4Model::ForwardBatch(int batch,
                                               const Data &inputIds,
        ...
        for (int i = 0; i < block_cnt; i++) {
            ...
            Linear(q, weight["model.layers." + std::to_string(i) + ".mlp.down_proj.weight"], Data(), k);
            std::string postMLPNormName = "model.layers." + std::to_string(i) + ".post_self_attn_layernorm.weight";
            if (weight.weight.find(postMLPNormName) != weight.weight.end())
                RMSNorm(k, this->weight[postMLPNormName], rms_norm_eps, k);
            AddTo(hiddenStates, k);
        }
